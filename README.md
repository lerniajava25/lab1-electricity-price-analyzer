...
• showApiIntegration(String selectedArea) ansvarar för att hämta elprisdata från API:et för det elområde som användaren har valt.
Javas inbyggda HttpClient används för HTTP-anropet. JSON-svaret läses som text och konverteras därefter med Jackson ObjectMapper till en List.

• fetchPricesSafely() hanterar fel som kan uppstå när elpriser hämtas från API:et.

• showPrices() tar emot listan med elpriser och skriver ut priserna med hjälp av en enhanced for-loop.

• showMenu() sköter programmets meny och användarens val. Användaren kan välja elområde SE1, SE2, SE3 eller SE4.

• isValidArea() kontrollerar att användaren har angett ett giltigt elområde.
...
Jag upplevde också att Java kräver mer explicit felhantering vid exempelvis API-anrop, där undantag som IOException och InterruptedException behöver hanteras.
